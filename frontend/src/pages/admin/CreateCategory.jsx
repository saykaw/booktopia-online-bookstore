...
const CreateCategory = () => {
  ...
  const handleSubmit = async (e)  =>{
    e.preventDefault()
    try {
      const {data} = await axios.post(`${apiKey}/api/v1/category/create-category`,{name})
      if(data?.success){
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `${apiKey}/api/v1/category/update-category/${selected._id}`,
        { name: updatedName }
      );
      if (data.success) {
  ...
  const handleDelete = async (id) => {
    try {
      const { data } = await axios.delete(
        `${apiKey}/api/v1/category/delete-category/${id}`
      );
      if (data.success) {
